...
export default async function Page({ params }: { params: Promise<Id> }) {
	...
	const [rPost, rComments]: Response[] = await Promise.all([
		fetch(`${host}/api/v1/placeholder/posts/${postId}?include=user`),
		fetch(`${host}/api/v1/placeholder/comments?include=user&postId=${postId}&page=${1}&pageSize=${100}`),
	]);
			...
			<div className={"grid gap-4 md:gap-12"}>
				<div className={"bg-background border border-input rounded-xl overflow-hidden w-full"}>
					...
					<div className={"grid gap-4 p-4"}>
						...
						<Separator />
						<h2 className={"text-4xl font-extrabold tracking-tight md:text-5xl first-letter:uppercase"}>
							{post.title}
						</h2>
						{post.tags.length && (
