...
> Probably the most powerful feature of Git is to run any scripts before or after git-operations. Although they are great for automation of things in your development process, managing them can be really cumbersome. That's where Husky comes in. :feet:
...
### How Husky and Conventional Commits Play Nicely Together
...
Commitlint is ready to go, to check how it works run the next command in your terminal and be ready to face with lot of errors :grin:
...
After initialization it will create a folder `.husky` in the root of your project, there you can see a `pre-commit` file, this is **a name of one of 13 client-side Git hooks** , check them all in the official [Git documentation](https://git-scm.com/docs/githooks) We can delete this file for now, or replace it's contents to `echo "pre-commit hook"`
...
That's all for now! Feel free to play with other hooks, for sure they will be helpful in many projects and many cases, and read [official documentation](https://typicode.github.io/husky) for more info about Husky :dog:
